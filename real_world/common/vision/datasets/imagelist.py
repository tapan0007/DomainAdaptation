...
class ImageList(datasets.VisionDataset):
    ...
    def __init__(self, root: str, classes: List[str], data_list_file: str, phase,
    ...
    def __getitem__(self, index: int) -> Tuple[Any, int, int, str]:
        """
        Args:
            index (int): Index
        ...
        feature_path = path+'.%s.pt' % self.phase
        if False:
            model = nn.Sequential(nn.AdaptiveAvgPool2d(output_size=(1,1)), nn.Flatten())
            img = torch.load(feature_path)
        ...
        return img, target, self.domain_index, path
